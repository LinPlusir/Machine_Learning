...
def predict(parameters, x_test, y_test):
    ...
    for k in range(0, n_cols):
        if output[0][k] == y_test[0][k] and output[1][k] == y_test[1][k] and output[2][k] == y_test[2][k] and output[0][k] == y_test[0][k] and output[3][k] == y_test[3][k] and output[4][k] == y_test[4][k] and output[5][k] == y_test[5][k]:
            count = count + 1
        else:
...
# 建立神经网络
def nn_model(X, Y, n_h, n_input, n_output, num_iterations=7400, print_cost=False):
    np.random.seed(3)
...
if __name__ == "__main__":
    ...
    X = data_set.iloc[:, 0:11].values.T          # 前十一列是特征，T表示转置
    Y = data_set.iloc[:, 11:].values.T           # 后六列是标签
    Y = Y.astype('uint8')

    parameters = nn_model(X, Y, n_h=23, n_input=11, n_output=6, num_iterations=7400, print_cost=True)
