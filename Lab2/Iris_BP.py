...
def init_parameters(n_x, n_h, n_y):
    ...
    #使用NumPy库生成形状为(n_h, n_x)的随机矩阵，每个元素都是从标准正态分布中随机抽取的。
    # 乘以0.01是为了将矩阵中的元素缩小，以便更好地进行后续计算。
...
def forward_propagation(X, parameters):
    ...
    # 通过前向传播来计算a2
    z1 = np.dot(w1, X) + b1     
    #这段代码是使用NumPy库中的dot函数计算矩阵w1和矩阵X的乘积，然后加上偏置向量b1。
    # 结果是一个形状为(n_h, m)的矩阵，n_h是隐藏层的神经元数量，m是输入样本的数量。
...
# 建立神经网络
def nn_model(X, Y, n_h, n_input, n_output, num_iterations=13000, print_cost=False):
    np.random.seed(3)
...
if __name__ == "__main__":
    ...
    # 开始训练
    # 输入4个节点，隐层20个节点，输出3个节点，迭代13000次
    parameters = nn_model(X, Y, n_h=20, n_input=4, n_output=3, num_iterations=13000, print_cost=True)
